...
import sharing.algorithms as shared_algorithms
import theano
...
class Optimizer():
    ...
    def fit(self, node_train, link_train, node_val=None, link_val=None):
        self.stack.set_training_data(link_train)
        if link_val is not None:
            self.stack.set_validation_data(link_val)
            # self.stack.set_val_node(node_val)
...
if __name__ == '__main__':
    ...
    opt = tf.build(loss, parameters, [X, Y])
    opt.predict = output
